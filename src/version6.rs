#![cfg_attr(docsrs, doc(cfg(feature = "v6")))]

//! This is an implementation of the [version 6 specification of PASETO](TODO).
...
impl Version for V6 {
    ...
    const SECRET_KEY: usize = 64;
    const PUBLIC_KEY: usize = 1312;
    const PUBLIC_SIG: usize = 7856;
    const LOCAL_NONCE: usize = 32;
    const LOCAL_TAG: usize = 32;
    const PUBLIC_HEADER: &'static str = "v6.public.";
    const LOCAL_HEADER: &'static str = "v6.local.";
    #[cfg(feature = "paserk")]
...
impl TryFrom<&AsymmetricSecretKey<V6>> for AsymmetricPublicKey<V6> {
    type Error = Error;

    fn try_from(value: &AsymmetricSecretKey<V6>) -> Result<Self, Self::Error> {
        let sk =
            SigningKey::<Sha2_128s>::try_from(secret_key.as_bytes()).map_err(|_| Error::Key)?;

        AsymmetricPublicKey::<V6>::from(&sk.verifying_key().to_bytes())
    }
}

impl Generate<AsymmetricKeyPair<V6>, V6> for AsymmetricKeyPair<V6> {
    fn generate() -> Result<AsymmetricKeyPair<V6>, Error> {
        let key = SigningKey::<Sha2_128s>::new(&mut OsRng);

        let public = AsymmetricPublicKey::<V6>::from(key.verifying_key().to_bytes())?;
        let secret = AsymmetricSecretKey::<V6>::from(key.to_bytes().as_slice())?;

        Ok(Self { public, secret })
    }
}

impl Generate<SymmetricKey<V6>, V6> for SymmetricKey<V6> {
    fn generate() -> Result<SymmetricKey<V6>, Error> {
        let mut rng_bytes = vec![0u8; V6::LOCAL_KEY];
        V6::validate_local_key(&rng_bytes)?;
...
/// PASETO v6 public tokens.
pub struct PublicToken;

impl PublicToken {
    /// The header and purpose for the public token: `v6.public.`.
    pub const HEADER: &'static str = "v6.public.";

    /// Create a public token.
    pub fn sign(
        secret_key: &AsymmetricSecretKey<V6>,
        message: &[u8],
        footer: Option<&[u8]>,
        ...
        // TODO: Is it supposed to be deterministic sigs or randomized??
        let sig = sk.sign(m2);
    ...
    pub fn verify(
        public_key: &AsymmetricPublicKey<V6>,
        token: &UntrustedToken<Public, V6>,
        footer: Option<&[u8]>,
        implicit_assert: Option<&[u8]>,
        ...
        let s = sm[m.len()..m.len() + V6::PUBLIC_SIG].as_ref();

        let m2 = pae::pae(&[public_key.as_bytes(), Self::HEADER.as_bytes(), m, f, i])?;
        let pk =
            VerifyingKey::<Sha2_128s>::try_from(public_key.as_bytes()).map_err(|_| Error::Key)?;

        debug_assert!(s.len() == V6::PUBLIC_SIG);
        // If the below fails, it is an invalid signature.
        let sig = Signature::<Sha2_128s>::try_from(s).map_err(|_| Error::TokenValidation)?;
...
/// PASETO v6 local tokens.
pub struct LocalToken;

impl LocalToken {
    /// The header and purpose for the local token: `v6.local.`.
    pub const HEADER: &'static str = "v6.local.";
    ...
    const M1_LEN: usize = V6::LOCAL_NONCE + Self::DOMAIN_SEPARATOR_ENC.as_bytes().len();
    const M2_LEN: usize = V6::LOCAL_NONCE + Self::DOMAIN_SEPARATOR_AUTH.as_bytes().len();
    ...
    fn key_split(sk: &[u8], n: &[u8]) -> Result<(EncKey, EncNonce, AuthKey), Error> {
        debug_assert_eq!(n.len(), V6::LOCAL_NONCE);
        debug_assert_eq!(sk.len(), V6::LOCAL_KEY);
        ...
        b2_ctx = Blake2b::new(&sk, V6::LOCAL_TAG).unwrap();
    ...
    pub(crate) fn encrypt_with_nonce(
        secret_key: &SymmetricKey<V6>,
        nonce: &[u8],
        message: &[u8],
        footer: Option<&[u8]>,
        implicit_assert: Option<&[u8]>,
    ) -> Result<String, Error> {
        debug_assert_eq!(nonce.len(), V6::LOCAL_NONCE);
        let f = footer.unwrap_or(&[]);
        let i = implicit_assert.unwrap_or(&[]);
        ...
        let mut b2_ctx = Blake2b::new(&auth_key, V6::LOCAL_TAG).unwrap();
        b2_ctx
        ...
        concat[concat_len - V6::LOCAL_TAG..].copy_from_slice(tag.unprotected_as_bytes());
    ...
    pub fn decrypt(
        ...
        let mut n: [u8; 32] = [0u8; V6::LOCAL_NONCE];
        n.copy_from_slice(nc[..V6::LOCAL_NONCE].as_ref());
        let c = token.untrusted_payload();
        let t = nc[nc.len() - V6::LOCAL_TAG..].as_ref();
...
mod test_vectors {
    ...
    fn test_local(test: &PasetoTest) {
        ...
        let sk =
            SymmetricKey::<V6>::from(&hex::decode(test.key.as_ref().unwrap()).unwrap()).unwrap();
        ...
        if test.expect_fail {
            if let Ok(ut) = UntrustedToken::<Local, V6>::try_from(&test.token) {
        ...
        let ut = UntrustedToken::<Local, V6>::try_from(&test.token).unwrap();
    ...
    fn test_public(test: &PasetoTest) {
        ...
        let sk = AsymmetricSecretKey::<V6>::from(
            &hex::decode(test.secret_key.as_ref().unwrap()).unwrap(),
        )
        .unwrap();
        let pk = AsymmetricPublicKey::<V6>::from(
        ...
        // payload is null when we expect failure
        if test.expect_fail {
            if let Ok(ut) = UntrustedToken::<Public, V6>::try_from(&test.token) {
        ...
        assert_eq!(actual, test.token, "Failed {:?}", test.name);
        let ut = UntrustedToken::<Public, V6>::try_from(&test.token).unwrap();
...
mod test_tokens {
    ...
    #[test]
    fn test_gen_keypair() {
        let kp = AsymmetricKeyPair::<V6>::generate().unwrap();

        let token = PublicToken::sign(&kp.secret, MESSAGE.as_bytes(), None, None).unwrap();

        let ut = UntrustedToken::<Public, V6>::try_from(&token).unwrap();
    ...
    fn test_untrusted_token_usage() {
        // Local
        let sk = SymmetricKey::<V6>::generate().unwrap();
        let token =
            LocalToken::encrypt(&sk, MESSAGE.as_bytes(), Some(FOOTER.as_bytes()), None).unwrap();

        let untrusted_token = UntrustedToken::<Local, V6>::try_from(token.as_str()).unwrap();
        ...
        let untrusted_token = UntrustedToken::<Public, V6>::try_from(token.as_str()).unwrap();
        assert!(
    ...
    fn test_roundtrip_local() {
        ...
        let ut = UntrustedToken::<Local, V6>::try_from(&token).unwrap();
    ...
    fn test_roundtrip_public() {
        let test_sk = AsymmetricSecretKey::<V6>::from(&TEST_SK_BYTES).unwrap();
        let test_pk = AsymmetricPublicKey::<V6>::from(&TEST_PK_BYTES).unwrap();
    ...
    #[test]
    fn footer_logic() {
        let test_local_sk = SymmetricKey::<V6>::from(&TEST_LOCAL_SK_BYTES).unwrap();
        let test_sk = AsymmetricSecretKey::<V6>::from(&TEST_SK_BYTES).unwrap();
        let test_pk = AsymmetricPublicKey::<V6>::from(&TEST_PK_BYTES).unwrap();
        let message =
        ...
        let actual_some = UntrustedToken::<Public, V6>::try_from(
            &PublicToken::sign(&test_sk, message, Some(FOOTER.as_bytes()), None).unwrap(),
        )
        .unwrap();
        let actual_none = UntrustedToken::<Public, V6>::try_from(
        ...
        let actual_some = UntrustedToken::<Local, V6>::try_from(
            &LocalToken::encrypt(&test_local_sk, message, Some(FOOTER.as_bytes()), None).unwrap(),
        )
        .unwrap();
        let actual_none = UntrustedToken::<Local, V6>::try_from(
    ...
    fn err_on_modified_footer() {
        ...
        assert_eq!(
            PublicToken::verify(
                &test_pk,
                &UntrustedToken::<Public, V6>::try_from(VALID_PUBLIC_TOKEN).unwrap(),
        ...
        assert_eq!(
            LocalToken::decrypt(
                &test_local_sk,
                &UntrustedToken::<Local, V6>::try_from(VALID_LOCAL_TOKEN).unwrap(),
                Some(FOOTER.replace("kid", "mid").as_bytes()),
                None
            )
            .unwrap_err(),
    ...
    fn err_on_no_footer_in_token_some_supplied() {
        ...
        assert_eq!(
            PublicToken::verify(
                &test_pk,
                &UntrustedToken::<Public, V6>::try_from(&invalid_public).unwrap(),
                Some(FOOTER.as_bytes()),
                None
            )
            .unwrap_err(),
        ...
        assert_eq!(
            LocalToken::decrypt(
                &test_local_sk,
                &UntrustedToken::<Local, V6>::try_from(&invalid_local).unwrap(),
                Some(FOOTER.as_bytes()),
                None
            )
            .unwrap_err(),
    ...
    fn err_on_invalid_public_secret_key() {
        let bad_pk = AsymmetricPublicKey::<V6>::from(&[0u8; 32]).unwrap();
    ...
    fn err_on_invalid_shared_secret_key() {
        let bad_local_sk = SymmetricKey::<V6>::from(&[0u8; 32]).unwrap();
...
mod test_keys {
    ...
    fn test_symmetric_gen() {
        let randomv = SymmetricKey::<V6>::generate().unwrap();
        assert_ne!(randomv.as_bytes(), &[0u8; 32]);
    }

    #[test]
    fn test_invalid_sizes() {
        assert!(AsymmetricSecretKey::<V6>::from(&[1u8; 63]).is_err());
        assert!(AsymmetricSecretKey::<V6>::from(&TEST_SK_BYTES).is_ok());
        assert!(AsymmetricSecretKey::<V6>::from(&[1u8; 65]).is_err());

        assert!(AsymmetricPublicKey::<V6>::from(&[1u8; 31]).is_err());
        assert!(AsymmetricPublicKey::<V6>::from(&[1u8; 32]).is_ok());
        assert!(AsymmetricPublicKey::<V6>::from(&[1u8; 33]).is_err());

        assert!(SymmetricKey::<V6>::from(&[0u8; 31]).is_err());
        assert!(SymmetricKey::<V6>::from(&[0u8; 32]).is_ok());
        assert!(SymmetricKey::<V6>::from(&[0u8; 33]).is_err());
    }

    #[test]
    fn try_from_secret_to_public() {
        let kpv4 = AsymmetricKeyPair::<V6>::generate().unwrap();
        let pubv4 = AsymmetricPublicKey::<V6>::try_from(&kpv4.secret).unwrap();
    ...
    #[test]
    fn test_trait_impls() {
        let debug = format!("{:?}", SymmetricKey::<V6>::generate().unwrap());
        assert_eq!(debug, "SymmetricKey {***OMITTED***}");

        let randomv = SymmetricKey::<V6>::generate().unwrap();
        let zero = SymmetricKey::<V6>::from(&[0u8; V6::LOCAL_KEY]).unwrap();
        assert_ne!(randomv, zero);

        let debug = format!("{:?}", AsymmetricKeyPair::<V6>::generate().unwrap().secret);
        assert_eq!(debug, "AsymmetricSecretKey {***OMITTED***}");

        let random1 = AsymmetricKeyPair::<V6>::generate().unwrap();
        let random2 = AsymmetricKeyPair::<V6>::generate().unwrap();
